...
python_src_path="/home/uccawkj/src_tool/SLAM/SLAM_V_2.2_upgrade_snapshot/slam_dipole_package/calc_slam_dipole.py"
...
MM_CNT=$(grep -A 1000 "Optimisation Meets Termination Condition, Final Configuration is" res.out | grep -A 1 "CONFIGURATION_XYZ_SC_INFO" | tail -1 | awk '{print $1}')
QM_CNT=$(grep -A 1000 "Optimisation Meets Termination Condition, Final Configuration is" res.out | grep -A 1 "CONFIGURATION_XYZ_SC_INFO" | tail -1 | awk '{print $2}')
...
python $python_src_path $POS_INTEGRAL $config_tmp $mo_tmp $species_tmp $MM_CNT $QM_CNT
